...
import com.auth.auth.annotation.PassToken;
...
import exceptions.MyTestException;
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object object) throws Exception {
        ...
        if (method.isAnnotationPresent(LoginToken.class)) {
            ...
            if (userLoginToken.required()) {
                // 执行认证
                if (token == null) {
                    throw new MyTestException("无token，请重新登录");
                }
                // 获取 token 中的 user id
                String userId;
                try {
                    ...
                    throw new MyTestException("401");
                }
                User user = userService.findUserById(userId);
                if (user == null) {
                    throw new MyTestException("用户不存在，请重新登录");
                }
                // 验证 token
